...
from src.utils.state import ReditState
...
async def set_edit_field(callback_query: CallbackQuery, user: User, state: FSMContext, lang: str):
    ...
    if cb_data == "name":
        ...
        data = await state.get_data()
        await state.set_state(ReditState.name)
        ...
        data = await state.get_data()
        await state.set_state(ReditState.age)
        msg = await callback_query.message.edit_text(txt, reply_markup=keyboard)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
    elif cb_data == "location":
        txt = "📍➡️ Пожалуйста, укажи свою локацию, чтобы мы могли предложить тебе людей поблизости!\n\nИли нажмите <b>'назад⬅️'</b>, чтобы отменить действие." if lang == "ru" else "📍➡️ Please share your location so we can suggest people nearby!\n\nOr press <b>'back⬅️'</b> to undo the action."
        data = await state.get_data()
        ...
        await state.set_state(ReditState.location)
        ...
        data = await state.get_data()
        await state.set_state(ReditState.gender)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
        ...
        data = await state.get_data()
        await state.set_state(ReditState.orientation)
        msg = await callback_query.message.edit_text(txt, reply_markup=keyboard1)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
    elif cb_data == "pref":
        ...
        data = await state.get_data()
        await state.set_state(ReditState.show)
        inline_keyboard = [
...
"""
        ...
        inline_keyboard.append([InlineKeyboardButton(text="Назад⬅️" if lang == "ru" else "Back⬅️",callback_data="fedit_back")])
        keyboard = InlineKeyboardMarkup(inline_keyboard=inline_keyboard)
        await state.set_state(ReditState.preferences)
        msg = await callback_query.message.edit_text(txt, reply_markup=keyboard)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
...
Select up to 5 hobbies that describe you. 🌟\n\n➡️ Click on the buttons to select your hobbies. Once you're done, click \"<b>Save✅</b>\".
    """
        await state.set_state(ReditState.hobbies)
        msg = await callback_query.message.edit_text(txt, reply_markup=keyboard)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
...
"📝➡️ Напиши короткое описание о себе, чтобы другие могли узнать тебя лучше! Укажи свои увлечения, интересы или что-то, что ты хотел бы рассказать о себе. 🌟\n\nИли нажми <b>'назад⬅️'</b>, чтобы отменить действие."
        ...
        data = await state.get_data()
        await state.set_state(ReditState.about)
        msg = await callback_query.message.edit_text(txt, reply_markup=keyboard)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
    elif cb_data == "descr":
...
"Press <b>'back⬅️'</b> to cancel the action."
        ...
        await state.set_state(ReditState.media)
        msg = await callback_query.message.edit_text(txt, reply_markup=keyboard)
        data["idmsg_edit"] = msg.message_id
        await state.update_data(data=data)
...
@router.message(ReditState.name)
...
@router.callback_query(lambda c: c.data.startswith("gender_"), ReditState.gender)
...
@router.callback_query(lambda c: c.data.startswith("orientation_"), ReditState.orientation)
...
@router.callback_query(lambda c: c.data.startswith("show_"), ReditState.show)
...
@router.callback_query(lambda c: "interest_" in c.data, ReditState.preferences)
...
@router.message(ReditState.location, lambda message: message.content_type == ContentType.LOCATION)
...
@router.message(ReditState.about)
...
@router.callback_query(lambda c: "intrs_" in c.data or c.data.startswith("intrs_page") or c.data == "intrs_done", ReditState.hobbies)
async def callback_handler(callback_query: CallbackQuery, user: User, state: FSMContext, lang: str):
